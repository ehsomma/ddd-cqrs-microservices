...
namespace Records.Persons.Infra.Reads.sql.Persons.GetPersonsByGenderAndBirthdate;

/// <summary>
/// Represents a read repository (reads from projection database).
/// </summary>
public class GetPersonsByGenderAndBirthdateRepository : ReadRepository, IGetPersonsByGenderAndBirthdateRepository
{
    #region Contructor
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string sql = @"
            ...
            WHERE
	                PE.Gender = @gender
	            AND PE.Birthdate BETWEEN @dateFrom AND @dateTo";

        // This way we can use cancellationToken.
        CommandDefinition cmd = new (sql, new { query.Gender, query.DateFrom, query.DateTo }, cancellationToken: cancellationToken);
